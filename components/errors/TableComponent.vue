<template>
    <div class="clients_section" id="clients">
            <div class="clients_wrapper">
                <div class="title">
                    <h2>Ingresa el código devuelto por SUNAT<span>.</span></h2>
                </div>
                <input class='search_input' placeholder='Ejem: 0100' @keyup='getCode' v-model='search' type='text'>
                <div class='rwd'>
                  <table class="rwd_auto">
                    ...
                    <tbody>
                      <tr v-for='tableCode in tableCodes' v-bind:key='tableCode.codigo' >
                        <td title='Click para copiar' @click='copyText(tableCode)'>{{tableCode.codigo}}</td>
                        <td title='Click para copiar' @click='copyText(tableCode)'>{{tableCode.descripcion}}</td>
                        <td title='Click para copiar' @click='copyText(tableCode)'>{{tableCode.tipo}}</td>
                        <td>
                          <button class='button button-primary'  @click='copyText(tableCode)' >Copiar</button>
                          <a target='_blank'  :href='"https://api.whatsapp.com/send?&text="+tableCode.codigo+" | "+tableCode.descripcion+" | "+tableCode.tipo'>Whatsapp</a>
                        </td>
                      </tr>                
                    </tbody>
                  </table>
...
export default {
    ...
    methods:{
      getCode(){
        this.tableCodes=[]
        if(this.search=='') this.tableCodes=this.codes
        else{
          for(let i=0; i<this.codes.length;i++){
            if(this.search==this.codes[i].codigo.substring(0,this.search.length)){
              this.tableCodes.push(this.codes[i])
            }
          }
        }
      },
      copyText(text){
        if (process.client) {
          var copyText = text.codigo+' | '+text.descripcion+' | '+text.tipo
